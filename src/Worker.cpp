...
void Worker::travel(){
	Logger *l = Logger::getInstance();
	try{
		l->log("Worker with id " +string(this->id) + " is traveling from " + string(this->locationStart)+ " to "+ string(this->locationEnd));
		...
		entrance.cerrar();
		l->log("Worker with id " +string(this->id) + " queued at " + string(this->locationStart));

		//lock semaphore until I arrive
		this->semTravel.wait();
		int loc = this->sharedMem.getLocation(this->id);
		if (loc != this->locationEnd){
			l->log("Worker with id " +string(this->id) + " was forced to get off at harbour " + string(loc));
		}else{
			l->log("Worker with id " +string(this->id) + " arrived at destination!");
		    sleep(8); //spend 8hs working...
		}
	}catch(string error){
		l->log("ERROR! " + "Worker with id " +string(this->id) + " :"+  string(strerror(errno)));
