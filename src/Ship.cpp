...
Ship::Ship(MemoriaCompartidaMapa &map, int harbour, int capacity):
Process(),
map(map),
harbour(harbour),
capacity(capacity){
    this->totalHarbours = map.totalHarbours();//this->map.leer(0);
...
void Ship::sail(){
    while(this->running()){
        ...
        int dstNextHarbour = map.distanceNextHarbour(this->harbour);//this->map.leer(this->harbour+1);
